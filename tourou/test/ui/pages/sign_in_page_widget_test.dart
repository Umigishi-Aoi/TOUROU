...
void main() {
  testGoldens('sign_in_page_golden_test', (WidgetTester tester) async {
    ...
    await tester.pumpWidgetBuilder(
      const TestWidget(platform: TargetPlatform.iOS, language: 'en'),
      surfaceSize: size6,
    );
    ...
    //第一引数はどのWidgetをビルドするのか指定、どのサイズにビルドするかがsurfaceSize
    await tester.pumpWidgetBuilder(
      const TestWidget(platform: TargetPlatform.iOS, language: 'ja'),
      surfaceSize: size6,
    );
    ...
    //第一引数はどのWidgetをビルドするのか指定、どのサイズにビルドするかがsurfaceSize
    await tester.pumpWidgetBuilder(
      const TestWidget(platform: TargetPlatform.android, language: 'en'),
      surfaceSize: size6,
    );
    ...
    //第一引数はどのWidgetをビルドするのか指定、どのサイズにビルドするかがsurfaceSize
    await tester.pumpWidgetBuilder(
      const TestWidget(platform: TargetPlatform.android, language: 'ja'),
      surfaceSize: size6,
    );
